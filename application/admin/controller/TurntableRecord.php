...
use app\admin\model\TurntableRecord AS MTurntableRecord;
use think\Request;

class TurntableRecord extends ApiCommon
{
    ...
    {   
        $query = new MTurntableRecord();

        $keyword = $request->get('keyword');
        if ($keyword) {
            $query->where('commodity_name', 'LIKE', '%' . $keyword . '%')
                  ->WhereOr('claim', 'LIKE', '%' . $keyword . '%') ;

        }
        $Artisan = $query->order('id desc')->paginate($request->get('limit'));
        return json($Artisan);
    }
    ...
    {   
        $TurntableRecord = MTurntableRecord::get($id);
        $TurntableRecord->is_grant = $request->post('is_grant');
        $res = $TurntableRecord->save();
        if ($res) {
    ...
    {
        $res = MTurntableRecord::get($id)->delete();
        if ($res) {
