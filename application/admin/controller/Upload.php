...
/*        header('Access-Control-Allow-Origin: *');
        ...
        $now = rand().$_SERVER['REQUEST_TIME'];//当前系统时间，比time()多5秒

        $image->save(ROOT_PATH . 'public' . DS . '/static/upload/image/'.date("Ym").''.$now.'.png');
        return json(['url'=>'static/upload/image/'.date("Ym").''.$now.'.png','message' => 'OK'],200);
        ##################先不用了###############
        $file = request()->file('file');
        if (!$file) {
          return resultArray(['error' => '请上传文件']);
        }
        $info = $file->validate(['ext'=>'jpg,png,gif'])->move(ROOT_PATH . 'public' . DS . '/static/upload/image');
        if ($info) {
            return json(['url'=>'static/upload/image/'.$info->getSaveName(),'message' => 'OK'],200);
        }
        ...
        try{  
            ...
            return json(['message'=>$e->getMessage()],203);
        }  
        // 返回给uploadify插件状态  
        if($image){  
            return json(['url'=>$image,'message' => 'OK'],200);      
        }else{  
        ...
        if ($upload_file['info']['http_code'] == 200) {
            unlink('static/upload/image/'.$info->getSaveName());
            return json(['url'=>'cms/'.$newName,'message' => 'OK'],200);
        }
            return json(['message'=>'上传失败'],203);
    }
