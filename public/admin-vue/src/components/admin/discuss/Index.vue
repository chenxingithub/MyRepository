                        ...
                        <div class="box-body">
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <el-button class="filter-item" type="primary" icon="search" v-if="listQuery.review == 0" @click="review">审核</el-button>
                                    <el-button class="filter-item" type="primary" icon="search" v-else @click="allList">全部</el-button>
                                    <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="edit">
                                        <router-link to="/admin/discuss/create" style="color:#FFF">{{ myCreateButtonText }}</router-link>
                                    </el-button>
                                </div>
                            </div>
                        </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            // 审核按钮
            review() {
                this.listQuery.review = 1;
                this.loadData();
            },
            allList() {
                this.listQuery.review = 0;
                this.loadData();
            },
            loadData() {
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除改信息, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/api/discuss/' + data.id)
                        .then((response) => {
            ...
            handleUpdate(data){
                this.$router.push('/admin/discuss/' + data.id + '/edit')
            },
            handleModifyStatus(data, status) {
                this.$axios.put('/api/discuss/' + data.id, {
                    status: !status,
                    action: 'changeStatus'
                })
                    .then((response) => {
