                            ...
                            <div class="row">
                                <div class="col-md-3">
                                 <!-- @keyup.enter.native="handleFilter"  监听回车事件执行handleFilter方法 -->
                                    <el-input style="width: 50%;" @keyup.enter.native="handleFilter" class="filter-item" placeholder="跳转地址" v-model="listQuery.keyword">
                                    </el-input>
                                </div>
                                <div class="col-md-3">
                            ...
                            <table class="table table-hover">
                                <tbody>
                                    ...
                                    <tr v-for="item in items">
                                        <td>{{ item['id'] }}</td>
                                        <td><span class="link-type" @click="handleUpdate(item)"><img :src="ImgPrefix+item['img_url']" style="width: 46px; height: 46px;" class="media-object img-circle"></span></td>
                                        <td >{{ item['jump_url'] }}</td>
                                        <td>{{ item['label'] }}</td>
                                        <td>{{ item['nature'] }}</td>
                                        <td>{{ item['is_recommend'] | isRecommendShow }}</td>
                                        <td>{{ item['created_at'] }}</td>
                                        <td>{{ item['updated_at'] }}</td>
                                        <td>
                                            <el-input style="width: 50%;" v-model="item['sort']" v-on:blur="handleModifySort(item)"></el-input>
                                        </td>
                                        <td>
                    ...
                    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible"  size="small">
                        <el-form class="small-space" :model="temp" :rules="rules" ref="temp"  label-position="left" label-width="100px" style='width: 800px; margin-left:100px;'>
                            ...
                            <el-form-item label="性质" prop="nature">
                                <el-input style="width: 50%;" v-if="dialogStatus=='update'" v-model="temp.nature"></el-input>
                                <el-input style="width: 50%;" v-else v-model="temp.nature"></el-input>
                            </el-form-item>
                            ...
                            <el-form-item label="描述" prop="describe">
                                <el-input style="width: 50%;" v-if="dialogStatus=='update'" v-model="temp.describe"></el-input>
                                <el-input style="width: 50%;" v-else v-model="temp.describe"></el-input>
                            </el-form-item>
                            <el-form-item label="热度" prop="degree_heat">
                                <el-input style="width: 50%;" v-if="dialogStatus=='update'" v-model="temp.degree_heat"></el-input>
                                <el-input style="width: 50%;" v-else v-model="temp.degree_heat"></el-input>
                            </el-form-item>
                            ...
                            <el-form-item label="推荐" prop="is_recommend">
                                  <el-select v-model="temp.is_recommend" placeholder="请选择">
  ...
  data() {
    ...
    return {
            ...
            rules: {
                ...
                is_recommend: [
                    { type:'number',required: true, message: '请选择是否推荐', trigger: 'change' }
                ],
                title: [
...
/*                this.$notify({
            ...
            handleCreate(){
                ...
                this.temp.label= ''
                this.temp.describe= ''
                this.temp.degree_heat= 0
                this.temp.title= ''
                this.temp.is_recommend= 0
            ...
            handleUpdate(data) {
                ...
                this.temp.nature= data.nature
                this.temp.label=  data.label
                this.temp.describe=  data.describe
                this.temp.title=  data.title
                this.temp.is_recommend=  data.is_recommend
                this.temp.degree_heat=  data.degree_heat
                this.dialogFormVisible = true
/*                this.$axios.get('/admin/user/' + data.id)
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除改用户, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/wechat-app/games/' + data.id)
                        .then((response) => {
                            if(response.status === 200){
                                this.$message({
                                    showClose: true,
            ...
            update(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.put('/wechat-app/games/' + this.temp.id, {
                            ...
                            is_recommend: this.temp.is_recommend,
                            degree_heat: this.temp.degree_heat,
            ...
            create(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('/wechat-app/games', {
                            img_url: this.temp.img_url,
                            title: this.temp.title,
                            jump_url: this.temp.jump_url,
                            nature: this.temp.nature,
            ...
            handleModifyStatus(data, status) {
                this.$axios.put('/wechat-app/games/' + data.id, {
                    status: status,
                    action: 'changeStatus'
                })
                    .then((response) => {
        ...
        filters: {
            ...
            isRecommendShow(isRecommend){
                const isRecommendMap = {
                    0:'否',
                    1:'是',
                };
                return isRecommendMap[parseInt(isRecommend)]
            }
