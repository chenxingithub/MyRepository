...
/*                this.$axios.get('/admin/user/' + data.id)
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除改用户, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/wechat-app/focus/' + data.id)
                        .then((response) => {
                            if(response.status === 200){
                                this.$message({
                                    showClose: true,
            ...
            update(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.put('/wechat-app/focus/' + this.temp.id, {
            ...
            create(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('/wechat-app/focus', {
                            img_url: this.temp.img_url,
            ...
            handleModifyStatus(data, status) {
                this.$axios.put('/wechat-app/focus/' + data.id, {
                    status: status,
                    action: 'changeStatus'
                })
                    .then((response) => {
