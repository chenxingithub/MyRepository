                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="edit">
                                        <router-link to="/admin/download/create" style="color:#FFF">{{ myCreateButtonText }}</router-link>
                                    </el-button>
                                </div>
                            </div>
                        </div>
        ...
        methods: {
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除改信息, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/api/download/' + data.id)
                        .then((response) => {
            ...
            handleUpdate(data){
                this.$router.push('/admin/download/' + data.id + '/edit')
            },
            handleModifyStatus(data, status) {
                this.$axios.put('/api/download/' + data.id, {
                    status: !status,
                    action: 'changeStatus'
                })
                    .then((response) => {
