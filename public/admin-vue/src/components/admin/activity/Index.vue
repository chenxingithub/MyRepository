                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="edit">
                                        <router-link to="/admin/activity/create" style="color:#FFF">{{ myCreateButtonText }}</router-link>
                                    </el-button>
                                </div>
                            </div>
                        </div>
                            ...
                            <table class="table table-hover">
                                <tbody>
                                    ...
                                    <tr v-for="item in items">
                                        ...
                                        <td>
                                            <span class="badge" @click="handleEnroll(item)">
                                            {{ item['enroll_count'] }}
                                            </span>
                                        </td>
        ...
        methods: {
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除改信息, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/api/activity/' + data.id)
                        .then((response) => {
            ...
            handleUpdate(data){
                this.$router.push('/admin/activity/' + data.id + '/edit')
            },
            handleEnroll(data){
                this.$router.push('/admin/activity/' + data.id + '/enroll')
            },
            handleModifyStatus(data, status) {
                this.$axios.put('/api/activity/' + data.id, {
                    status: !status,
                    action: 'changeStatus'
                })
                    .then((response) => {
