    ...
    export default {
        ...
        methods: {
            loadData() {
                ...
                var url = '/api/activity/'+this.$route.params.id+'/enroll';
                this.$axios.get(url, {
            ...
            handleDelete(data) {
                this.$confirm('此操作将删除该账号报名, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/api/activity/' + this.$route.params.id+'/enroll', {
                        ...
                        .then((response) => {
                            if(response.status === 204){
                                this.$message({
                                    showClose: true,
                                    message: (response.data.length === 0?'删除报名成功':response.data),
                                    type: 'success'
                                });
                                this.loadData();
                            }else{
                                this.$message({
                                    showClose: true,
                                    message: (response.data.error.data === 0?'删除报名失败':response.data.error.data),
                                    type: 'error'
                                });
                            }
                        }, (response) => {
            ...
            handleDownload(){
                window.open(window.location.protocol+'//'+window.location.host+'/admin/activity/excel/'+this.$route.params.id+'?q='+this.listQuery.q)
            }
        }
    }
</script>