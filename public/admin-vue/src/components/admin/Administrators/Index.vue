...
<div class="content-wrapper" style="min-height: 901px;margin-top: 4.5rem;" v-loading="loading"
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><router-link to="/admin/administrators" tag="a"><i class="fa fa-user"></i> 管理员</router-link></li>
                    ...
                    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible"  size="small">
                        <el-form class="small-space" :model="temp" :rules="rules" ref="temp"  label-position="left" label-width="100px" style='width: 400px; margin-left:100px;'>
                            ...
                            <el-form-item label="角色" prop="roles_name">
                                  <el-select v-model="temp.roles_value" placeholder="请选择">
                                    <el-option
                            ...
                            <el-form-item label="邮箱" prop="email">
                                <el-input type="email" v-if="dialogStatus=='update'" v-model="temp.email"></el-input>
                                <el-input type="email" v-else v-model="temp.email"></el-input>
                            </el-form-item>
                            <el-form-item label="昵称" prop="nickname">
                                <el-input v-if="dialogStatus=='update'" v-model="temp.nickname"></el-input>
                                <el-input v-else v-model="temp.nickname"></el-input>
                            </el-form-item>
                            <el-form-item label="密码" prop="password" v-if="dialogStatus=='create'">
                                <el-input type="password" v-model="temp.password"></el-input>
                            </el-form-item>
                            <el-form-item label="确认密码" prop="password_confirmation" v-if="dialogStatus=='create'">
...
export default {
  ...
  methods: {
            loadData() {
            	this.loading = true
                let url = '/admin/system/admin/get';
                this.$axios.get(url, {
            ...
            handleUpdate(data) {
                ...
                this.$axios.get('/admin/system/admin/' + data.id)
                    .then((response) => {
                        if(response.status === 200){
                            this.temp.roles_value= response.data.roles_value
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除改用户, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/admin/system/admin/' + data.id)
                        .then((response) => {
                            if(response.status === 200){
                                this.$message({
                                    showClose: true,
            ...
            create(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('/admin/system/admin/', {
                            name: this.temp.name,
                            nickname: this.temp.nickname,
                            email: this.temp.email,
                            avatar: this.temp.avatar,
