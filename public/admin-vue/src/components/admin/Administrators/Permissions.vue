...
<div class="content-wrapper" style="min-height: 901px;margin-top: 4.5rem;" v-loading="loading"
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><router-link to="/admin/administrators" tag="a"><i class="fa fa-user"></i> 权限</router-link></li>
                            ...
                            <div class="row">
                                <div class="col-md-3">
                            	 <!-- @keyup.enter.native="handleFilter"  监听回车事件执行handleFilter方法 -->
                                    <el-input @keyup.enter.native="handleFilter" class="filter-item" placeholder="路由别名、名称" v-model="listQuery.keyword">
                                    </el-input>
                                </div>
                                <div class="col-md-9">
                            ...
                            <table class="table table-hover">
                                <tbody>
                                    <tr>
                                        ...
                                        <th style="word-break: break-word;">路由别名</th>
                                        <th>名称</th>
                                        <th>描述</th>
                                        <th>创建时间</th>
                                        <th>更新时间</th>
                    ...
                    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible"  size="small">
                        <el-form class="small-space" :model="temp" :rules="rules" ref="temp"  label-position="left" label-width="100px" style='width: 400px; margin-left:100px;'>
                            ...
                            <el-form-item label="图标" prop="icon" >
                                <el-input v-if="dialogStatus=='update'" v-model="temp.icon"></el-input>
                                <el-input v-else v-model="temp.icon"></el-input>
                            </el-form-item>
                            <el-form-item  prop="is_menu" >
                            <el-checkbox v-if="dialogStatus=='update'" v-model="temp.is_menu" >是否为菜单</el-checkbox>
                            <el-checkbox v-else v-model="temp.is_menu" >是否为菜单</el-checkbox>
                            </el-form-item>
                        </el-form>
...
export default {
  ...
  methods: {
            loadData() {
            	this.loading = true
                let url = '/admin/system/permissions/get';
                this.$axios.get(url, {
            ...
            handleCreate(){
                ...
                this.$axios.get(url, {
                    ...
                    params: {
                      ...
                      this.temp.options= response.data;
                      this.temp.options.push({
                            id: 0,
                            display_name:'没有父级'
                        });
                      console.log(this.temp.options);
                      this.loading = false
                })
            ...
            handleUpdate(data) {
                ...
                this.$axios.get('/admin/system/permissions/' + data.id)
                    .then((response) => {
                        if(response.status === 200){
                            ...
                            this.temp.icon = response.data.icon
                            this.temp.parent_id = response.data.parent_id
                            if (response.data.is_show==1) {
                                this.temp.is_menu = true
                            }else{
                                this.temp.is_menu = false
                            }
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除该权限, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/admin/system/permissions/' + data.id)
                        .then((response) => {
                            if(response.status === 200){
                                this.$message({
                                    showClose: true,
            ...
            update(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.put('/admin/system/permissions/' + this.temp.id, {
                            name: this.temp.name,
            ...
            create(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('/admin/system/permissions', {
                            name: this.temp.name,
                            display_name: this.temp.display_name,
                            description: this.temp.description,
                            icon: this.temp.icon,
