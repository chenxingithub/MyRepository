...
<div class="content-wrapper" style="min-height: 901px;margin-top: 4.5rem;" v-loading="loading"
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        <li ><a href="javascript:void(0)" v-on:click="PreviousPage" ><i class="fa fa-tv"></i>返回文章列表</a></li>
                    ...
                    <div class="box box-info">
                      ...
                      <el-form-item label="文章分类" prop="artisan_type_id">
                       ...
                       <el-form-item label="终端" prop="terminal">
                            ...
                            <el-form-item label="文章标签" prop="jump_url">
...
export default {
  ...
  methods: {
            loadData() {
                this.temp.content = "33e23123235r47633";
            	this.loading = true
                let url = '/admin/artisan/get/'+this.$route.query.id;
                this.$axios.get(url, {
                    headers: this.$store.state.headers,
                }).then(response => {
                const kk = response.data;
                this.temp.id = this.$route.query.id;
                this.temp.content = response.data.content;
                this.temp.title = response.data.title;
                this.temp.label = response.data.label;
                this.temp.artisan_type_id = response.data.artisan_type_id;
                this.loading = false;
                })
            },
            ...
            update(formName){
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.put('/artisan/put/' + this.temp.id, {
            ...
            create(formName){
                ...
                this.temp.terminal= this.temp.terminal.toString();
                let content = this.$refs.ue.getUEContent();
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('/admin/artisan/post', {
                            id: this.temp.id,
                            title: this.temp.title,
                            artisan_type_id: this.temp.artisan_type_id,
                            name: this.temp.name,
        ...
        created() {
            ...
            this.temp = this.$route.query;
            this.temp.terminal= this.temp.terminal.toString();
            let url = '/artisan-type/get';
            this.$axios.get(url, {
                headers: this.$store.state.headers,
                params: {
                    query_type: 1,
