                            ...
                            <div class="row">
                                <div class="col-md-3">
                            	 <!-- @keyup.enter.native="handleFilter"  监听回车事件执行handleFilter方法 -->
                                    <el-input style="width: 50%;" @keyup.enter.native="handleFilter" class="filter-item" placeholder="名称" v-model="listQuery.keyword">
                                    </el-input>
                                </div>
                                <div class="col-md-3">
                                    状态：
                    ...
                    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible"  size="small">
                        <el-form class="small-space" :model="temp" :rules="rules" ref="temp"  label-position="left" label-width="100px" style='width: 800px; margin-left:100px;'>
                            <el-form-item label="文章分类名称" prop="name">
                             ...
                             <el-form-item label="父级分类" prop="p_id">
                                <el-select v-model="temp.p_id" placeholder="请选择">
                                            <el-option
...
/*                this.$axios.get('/admin/user/' + data.id)
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除该焦点图, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/artisan-type/delete/' + data.id)
                        .then((response) => {
                            if(response.status === 200){
                                this.$message({
                                    showClose: true,
            ...
            handleModifyStatus(data, status) {
                this.$axios.put('/artisan-type/put/' + data.id, {
                    status: status,
                    action: 'changeStatus'
                })
                    .then((response) => {
