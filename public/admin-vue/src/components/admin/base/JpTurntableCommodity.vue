                            ...
                            <div class="row">
                                <div class="col-md-3">
                            	 <!-- @keyup.enter.native="handleFilter"  监听回车事件执行handleFilter方法 -->
                                    <el-input style="width: 50%;" @keyup.enter.native="handleFilter" class="filter-item" placeholder="奖品名称" v-model="listQuery.keyword">
                                    </el-input>
                                </div>
                                <div class="col-md-3">
                            ...
                            <table class="table table-hover">
                                <tbody>
                                    ...
                                    <tr v-for="item in items">
                                        <td>{{ item['id'] }}</td>
                                        <td><span class="link-type" @click="handleUpdate(item)"><img :src="ImgPrefix+item['img']" style="width: 46px; height: 46px;" class="media-object img-circle"></span></td>
                                        <td >{{ item['name'] }}</td>
                                        ...
                                        <th>{{item['currency_num']}}</th>
<!--                                         <td>
    <el-input style="width: 50%;" v-model="item['sort']" v-on:blur="handleModifySort(item)"></el-input>
</td> -->
                                        <td>
                                            <el-tag :type="item['is_delete'] | statusFilter">{{ item['is_delete'] | statusShow }}</el-tag>
                                        </td>
                                        <td>
<!--                                             <el-button v-if="parseInt(item['is_delete'])==1" size="small" type="success" @click="handleModifyStatus(item,0)">正常
</el-button>
<el-button v-if="parseInt(item['is_delete'])==0" size="small" @click="handleModifyStatus(item,1)">锁定
</el-button> -->
                    ...
                    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible"  size="small">
                        <el-form class="small-space" :model="temp" :rules="rules" ref="temp"  label-position="left" label-width="100px" style='width: 800px; margin-left:100px;'>
                            ...
                            <el-form-item label="货币值" prop="currency_num">
                                <el-input style="width: 50%;" v-if="dialogStatus=='update'" v-model="temp.currency_num"></el-input>
                                <el-input style="width: 50%;" v-else v-model="temp.currency_num"></el-input>
                            </el-form-item>
                        </el-form>
...
export default {
  ...
  methods: {
            loadData() {
            	this.loading = true
                let url = '/jp-turntable-commodity';
                this.$axios.get(url, {
...
/*                this.$notify({
            ...
            handleUpdate(data) {
                ...
                this.temp.currency_num = data.currency_num
...
/*                this.$axios.get('/admin/user/' + data.id)
            ...
            handleDelete(data) {
                this.$confirm('此操作将永久删除改用户, 是否继续?', '提示', {
                    ...
                    this.$axios.delete('/jp-turntable-commodity/' + data.id)
                        .then((response) => {
                            if(response.status === 200){
                                this.$message({
                                    showClose: true,
            ...
            create(formName){
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$axios.post('/jp-turntable-commodity', {
                            img: this.temp.img,
                            name: this.temp.name,
                            probability: this.temp.probability,
                            total_number: this.temp.total_number,
