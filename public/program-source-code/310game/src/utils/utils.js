...
//验证码倒计时
function timeDown(time, dom) {
  // console.log(dom);
  dom.style.color = '#7f7c7c';
  dom.disabled = ' disabled';

  let setTime =  setInterval(()=>{
    if(time <= 0) {
      clearInterval(setTime);
      dom.style.color = '#31aae2';
      dom.disabled = 'enabled';
      dom.innerHTML = '获取验证码';
    } else {
      dom.innerHTML = time + 's后重新获取';
      time--;
    }
  },1000);
};
...
//修改密码验证码倒计时
function bindTelTimeDown(time, dom) {
  // console.log(dom);
  dom.style.color = '#d0d0d0';
  dom.disabled = ' disabled';

  let setTime =  setInterval(()=>{
    if(time <= 0) {
      clearInterval(setTime);
      dom.style.color = '#fffff';
      dom.disabled = 'enabled';
      dom.innerHTML = '获取验证码';
    } else {
      dom.innerHTML = time + 's后重新获取';
